...
def CIFAR10Model(Img, ImageSize, MiniBatchSize):
    ...
    #############################
    ip = Img
    ip = tf.layers.conv2d(ip, name='conv1', filters = 32, kernel_size = 5, activation = None)
    ip = tf.layers.batch_normalization(ip)
    ip = tf.nn.relu(ip, name='relu1')


    ip = tf.layers.conv2d(ip, name='conv2', filters = 64, kernel_size = 3, activation = None)
    ip = tf.layers.batch_normalization(ip)
    ip = tf.nn.relu(ip, name='relu2')

    ip = tf.layers.conv2d(ip, name='conv3', filters = 64, kernel_size = 3, activation = None)
    ip = tf.layers.batch_normalization(ip)
